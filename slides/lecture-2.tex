...
\begin{frame}{Аналитическая машина}
    ...
    \item Программируемая машина:
        \begin{itemize}
            ...
            \item I/O (перфокарты).
        \end{itemize}
        \onslide<3>{\item Luigi Menabrea, 1943,\\
...
\begin{itemize}
    \item Z3 (1941) — Конрад Цузе, Германия;
        \\ \onslide<3->{{\small binary, floating-point, булева логика,
             без условий.}}
    \item Компьютер Атанасова-Берри (ABC, 1942), США;
        \\ \onslide<4->{{\small binary, конденсаторы для памяти, решает СЛУ.}}

    \item Harvard Mark I (1944) — Говард Айкен, IBM;
        \\ \onslide<5->{{\small decimal, cf.~Ч.~Бэббидж, «гарвардская архитектура», loops.}}
